...
You have finished writing a Python program for matching with ActiveMatcher. You can see the complete Python file [here](https://github.com/anhaidgroup/active_matcher/blob/main/examples/am_local_example.py).

To run this program, open a terminal and navigate to the directory that you wrote your 'am_local_example.py' file in. Then run the following command, which will output 'true_positives', 'precision', 'recall', and 'f1':
...
As labeling occurs in seed selection and active learning, ActiveMatcher saves labeled examples. This way, if a user is unable to complete labeling in one sitting for any reasons, ActiveMatcher can load in the examples that have already been labeled (as we discuss below).
